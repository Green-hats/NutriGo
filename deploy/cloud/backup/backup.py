...
def prune(destination: Path, keep: int) -> None:
    if keep < 1:
        raise ValueError("Keep at least one verified backup")
    previous = sorted(
        p
        for p in destination.glob("snapshot-*")
...
def create(backend: Path, agent: Path, destination: Path, keep: int = 14, *, prune_old: bool = True) -> Path:
    if keep < 1:
        raise ValueError("Keep at least one verified backup")
    ...
    with (destination / ".lock").open("a") as lock:
        ...
        try:
            ...
            databases = {
                name: database_counts(staging / name) for name in ["backend/data.db", "agent/agent.db"]
            }
            files = {
                str(p.relative_to(staging)): digest(p) for p in sorted(staging.rglob("*")) if p.is_file()
            }
            manifest = {
                "format": 1,
...
def main() -> None:
    ...
    backup.add_argument(
        "--no-prune", action="store_true", help="Defer retention until optional offsite verification succeeds"
    )
    ...
    if args.command == "create":
        print(create(args.backend, args.agent, args.destination, args.keep, prune_old=not args.no_prune))
