...
核对日期：2026-09-20；数据层运行实现与生产部署基线为 `6ddfe4d3`。本文说明当前实现、已启用的运维措施与剩余缺口。部署命令见[云端运维](../deploy/cloud/README.md)，接口字段见 [Go API](../backend/API.md)。
...
照片已实施个人/全站配额、在途容量预留及磁盘低水位检查；新删除任务在清理完成前继续计入配额，默认值及旧任务兼容边界见[请求与资源限制](RESOURCE_LIMITS.md)。目前仍缺少统一账号注销与导出、跨会话/回执/备份的删除政策、数据库版本化迁移、原生安全凭据存储和离线同步。异地存储、通知接收端及外部监控需要真实资源接入；完成接入后还应演练从独立环境完整恢复。优先级见[路线图](ROADMAP.md)。
