...
核对日期：2026-09-20。公开安装包为 [Android 1.0.0 正式版](https://github.com/Green-hats/NutriGo/releases/tag/android-v1.0.0)，生产服务与 Release 均已部署提交 `6ddfe4d3`。源码提交、云端部署和 APK 仍是独立交付物，文档或纯服务端更新不意味着必须重新发布 APK。
...
| AI | POST + SSE 流式对话、停止和重试、会话管理、5 个工具、展开工具详情 | `thinking` 取决于模型返回；旧 GET 聊天客户端不兼容；无后台生成 / 推送 |
...
[1.0.0 发布 CI](https://github.com/Green-hats/NutriGo/actions/runs/35484622601) 的版本校验、六组质量门禁、签名与发布任务全部通过。该记录对应提交 `6ddfe4d3`，不代表后续提交自动通过，也不代替真机体验和模型效果评估。检查命令见[贡献指南](../CONTRIBUTING.zh-CN.md)。
...
- 后端和 Agent 接口需先兼容新客户端，再发布 APK。旧 GET 聊天因正文进入 URL 的隐私风险已停止服务；账号、档案、日记与旧 CLIP 接口继续兼容，后续移除必须另设迁移计划。
