...
核对日期：2026-09-20，Go 运行实现与生产部署基线 `6ddfe4d3`。Go + Gin + GORM + SQLite 提供账号、档案、饮食、汇总、图片及令牌管理；开发端口为 `3333`。生产由 Caddy 转发 `/api/*`，Go 端口不直接公开。字段、状态码与完整路由见 [API 文档](../backend/API.md)。
